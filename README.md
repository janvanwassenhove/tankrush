...
- Keyboard and multi-touch controls, course minimap, optional procedural audio, PWA manifest and versioned same-origin offline cache.
...
Touch controls appear on coarse-pointer devices. Offline play is available after one successful online load and service-worker installation. System fonts are used when Google Fonts is unavailable. PWA install UI depends on the browser; iOS uses Share → Add to Home Screen.
...
- `dist/src/main.js`: input, lifecycle, HUD, touch and audio.
