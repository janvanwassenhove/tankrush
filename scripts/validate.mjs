...
const html=await readFile('dist/index.html','utf8');for(const m of html.matchAll(/(?:src|href)="(\.\/[^"]+)"/g)){await stat(resolve('dist',m[1]));}
...
const sw=await readFile('dist/sw.js','utf8');const assets=sw.match(/const ASSETS=(\[[\s\S]*?\]);/)[1];for(const m of assets.matchAll(/'\.\/([^']*)'/g))await stat(resolve('dist',m[1]));
