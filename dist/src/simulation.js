...
export function terrainHeight(x,z){x/=2.8;z/=2.8;return 1.1+Math.sin(x*.105)*1.25+Math.cos(z*.16)*.65+Math.sin((x+z)*.13)*.45+Math.exp(-((x+18)**2+(z-8)**2)/60)*3.4;}
export function pathPoint(mode,t,time=0){
  const a=t*TAU, x=Math.cos(a)*HABITAT.trackX+Math.sin(3*a)*4, z=Math.sin(a)*HABITAT.trackZ;
...
export function surfaceAt(x,z){x/=2.8;z/=2.8;if(x < -17&&z>1&&z<15)return 'rock';if(z < -12&&x>0)return 'sand';if(x>15&&z>5)return 'plant';return 'soil';}
...
export class Simulation {
 ...
 resetRacer(r=this.racers[0]){const t=r.passed?(r.gate+this.config.gates-1)%this.config.gates/this.config.gates:-.026,p=pathPoint(this.mode,t,this.time);Object.assign(r,p,{yaw:pathHeading(this.mode,t),vx:0,vy:0,vz:0,speed:0,steering:0,pitch:0,roll:0,immune:2,slow:0,previous:{...p}});if(!r.id)this.emit('Terug op koers · 3 seconden tijdstraf');if(!r.id)this.elapsed+=3;}
 ...
 move(r,dt,input){
  ...
  const bx=clamp(r.x,-HABITAT.halfWidth+3,HABITAT.halfWidth-3),bz=clamp(r.z,-HABITAT.halfDepth+3,HABITAT.halfDepth-3);if(bx!==r.x||bz!==r.z){r.x=bx;r.z=bz;r.speed*=.8;r.vx*=.6;r.vz*=.6;if(r.id===0&&r.immune<=0){this.emit('Glaswand! Stuur terug naar de poort.');r.immune=1;}}
 }
